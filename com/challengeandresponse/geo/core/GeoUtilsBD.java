...
/**
 * BigDecimal version of geo utils... this is the original class. the new class GeoUtils works with regular
 * decimal values.
 * <tt>GeoUtilsBD/GeoUtils</tt> classes supply static methods for handling calculations involving points and areas on Earth.
 */

public class GeoUtilsBD {
	...
	public static double heading(Point2D.Double origin, Point2D.Double end){
		 ...
		 *cartesian coordinates with "origin" point at the origin
		 ...
		 */
		double dist=Math.max(distance(origin,end),1);
	 ...
	 * @param origin the starting point
	 * @param line_end the line segment described by two points on Earth.
	...
	public static double heading(Point2D.Double origin, Line2D.Double line_end){
		...
		headingP1P2= heading(p1,p2);
		headingP1O = heading(p1,origin);
		headingP2P1= heading(p2,p1);
		headingP2O = heading(p2,origin);
		...
		if((angleOP1P2>90)||(angleOP2P1>90)){
			...
			if(d1<d2)
				return heading(origin,p1);
			else 
				return heading(origin,p2);
		}
		else {
	 ...
	 * @param origin the starting point
	 * @param poly_end the polygon representing some area on Earth.
	 ...
	 */
	public static double heading(Point2D.Double origin, Polygon poly_end){
...
//		double drlon;
		...
		rBD =new BigDecimal(R_A / (1 - (GEO_E2 * Math.pow(Math.sin(rlat),2))));
...
//		toWHFormat(location);
	 ...
	 *@param range the range of projection
	 */
	public static void project(Polygon location_poly, double angle, double range){
	...
	/** 
	 * Version of distance() that uses bare doubles rather than Point2D.Double as args
	 */
	public static double distance(double lat1, double lon1, double lat2, double lon2) {
		return distance(new Point2D.Double(lat1,lon1), new Point2D.Double(lat2,lon2));
	}


	/**
	 * Calculates the distance between two points, <tt>origin</tt> and <tt>end</tt>. 
	...
	public static double distance(Point2D.Double origin, Point2D.Double end){
		...
		// measures come in as degrees, the trig functions want radians
		rlat1=Math.toRadians(origin.x); 
		rlat2=Math.toRadians(end.x);
		rlon1=Math.toRadians(origin.y);
		rlon2=Math.toRadians(end.y);
		...
		/* calculate r, the earth's radius at this latitude
		 * r = (GEO_A * Math.sqrt(1 - GEO_E2)) / (1 - (GEO_E2 * Math.pow(Math.sin(Math.toRadians(this.lat)),2)));
		 * solved first part of this (see class static var R_A) because it's constant
		 */
		rBD = new BigDecimal(R_A / (1 - (GEO_E2 * Math.pow(Math.sin(rlat1),2))));
	...
	public static double distance(Point2D.Double origin, Line2D.Double line_end){
		...
		if((angleOP1P2>90)||(angleOP2P1>90)){
			...
			return Math.min(distance(origin,p1),distance(origin,p2));
		}
		else {
			//System.out.println("not obtuse");
			//return the distance to P1P2 line
			//System.out.println("distance: "+distance(origin,p1));
			//System.out.println("sin : "+Math.sin(Math.toRadians(angleOP1P2)));
			BigDecimal temp_dist = new BigDecimal(distance(origin,p1));
	 ...
	 *@param poly_end a polygon describing some geographical area
	 *@return Geographical distance between <tt>origin</tt> and <tt>end_poly</tt>, given with double precision
	 */
	public static double distance(Point2D.Double origin, Polygon poly_end){
