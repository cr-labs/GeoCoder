...
/**
 * <tt>Polygon</tt> represents a geographical area.  <tt>Polygon</tt> supplies methods for manipulating such area, 
...
public class Polygon{
    ...
    /**
     *Constructs a new <tt>Polygon</tt> object.
     *@param poly_descriptor a string that contains the description of all the vertices of <tt>Polygon</tt> 
     *in the form <i>(lat1,lon1),(lat2,lon2)....(latN,lonN)</i>.  This constructor is meant to be used to reconstruct
     * <tt>Polygon</tt> object when it is retrieved from Postgres. This method does not verify that
     ...
     *the constructor will in most cases produce zero-vertex <tt>Polygon</tt> object.  
     */
    public Polygon(String poly_descriptor){
    ...
    /**
     *Constructs a new square <tt>Polygon</tt> object , 2*radius x 2*radius, centered at <tt>client_location</tt>. 
     *Its sides are 2*radius and its center is described by <tt>client_location</tt>.
     *This constructor is designed for radial searches.
     *@param client_location a geographical point which will also mark the center of constructed <tt>Polygon</tt>.
     *@param radius a radius of the largest circle that could be inscribed into constructed <tt>Polygon</tt> - 
     * effectively, it is half of the side length
     */ 
    public Polygon(Point2D.Double client_location, double radius){
    ...
    /**
     *Constructs a new rectangular <tt>Polygon</tt> object, rectangular shape, <tt>width</tt> x <tt>length</tt>,
     * tilted by an angle given by <tt>heading</tt>.
     ...
     */  
    public Polygon(Point2D.Double client_location, double heading, double width, double length){
	...
	GeoUtilsBD.project(poly_vertex,poly_side_angle,half_width);
	coordinates[0]=new Point2D.Double(poly_vertex.getX(),poly_vertex.getY());
	//calculate a second vertex
	poly_side_angle=heading;
	GeoUtilsBD.project(poly_vertex,poly_side_angle,length);
	coordinates[1]=new Point2D.Double(poly_vertex.getX(),poly_vertex.getY());
	//calculate a third vertex
	poly_side_angle=((heading+90)%360);
	GeoUtilsBD.project(poly_vertex,poly_side_angle,width);
	coordinates[2]=new Point2D.Double(poly_vertex.getX(),poly_vertex.getY());
	//calculate a fourth vertex
    ...
    /**
     * Projects this <tt>Polygon</tt> by <tt>project_heading</tt> and <tt>project_range</tt>.
...
//	Point2D.Double temp_loc;
	for(int i=0;i<coordinates.length;i++){
	    GeoUtilsBD.project(coordinates[i],project_heading,project_range);
	}
    }
    /**
     * Calculates the geographical distance between this <tt>Polygon</tt> and a location described by <tt>origin</tt>.
     * @param origin some point on Earth
     * @return the geographical distance from this <tt>Polygon</tt> to <tt>origin</tt>, in meters.  
     *If <tt>origin</tt> lies within this <tt>Polygon</tt>, return 0.
     */
    public double distance(Point2D.Double origin){
	//if this <tt>Polygon</tt> has one vertex => return the distance to that vertex
	if (coordinates.length == 1){
	    return GeoUtilsBD.distance(origin, coordinates[0]);
	} else {
	    //if origin inside this <tt>Polygon</tt> => return 0
		...
		double d=GeoUtilsBD.distance(origin, new Line2D.Double(coordinates[coordinates.length-1],coordinates[0]));
		//System.out.println("temp_d: "+d);
		double temp_d;
		for (int i=1;i<coordinates.length;i++){
		    temp_d=GeoUtilsBD.distance(origin, new Line2D.Double(coordinates[i-1],coordinates[i]));
    ...
    /**
     * Calculate the heading from a location described by <tt>origin</tt> to this <tt>Polygon</tt>.  
     * @param origin some point on the Earth
     * @return Heading, an angle between a meridian passing through <tt>origin</tt> 
     * and a line from <tt>origin</tt> to this <tt>Polygon</tt>.
     */
    public double heading(Point2D.Double origin){
	//if this Polygon has one vertex => return the heading to that vertex
	if (coordinates.length == 1){
	    return GeoUtilsBD.heading(origin, coordinates[0]);
	} else {
	    //if origin inside this Polygon => return 0
	    if (this.contains(origin)){
		return 0;
	    } else {
		...
		return GeoUtilsBD.heading(origin, closest_side);
	    }
	}
    }
    /**
     * Checks if <tt>point</tt> lies inside of this <tt>Polygon</tt>.
     * @param point some location on Earth
     * @return boolean value indicating whether <tt>point</tt> lies within the boundaries of this <tt>Polygon</tt>
     */
    ...
    /**
     * Checks if the area of this <tt>Polygon</tt> overlaps a 0<sup>o</sup> meridian.
     * @return a boolean value indicating whether this <tt>Polygon</tt> overlaps 0<sup>o</sup> meridian.
     */
    public boolean zeroCrossing(){
	return zero_crossing;
    }
    /**
     * Returns new <tt>Polygon</tt> objects, produced by cutting this <tt>Polygon</tt> 
     * in two along 0<sup>o</sup> meridian.
     * @return An array of new <tt>Polygon</tt> objects created by splitting 
     * this <tt>Polygon</tt> in two along 0<sup>0</sup> meridian.  
     * If this <tt>Polygon</tt> does not overlap 0<sup>o</sup> meridian, a clone of this <tt>Polygon</tt> is returned.
     */ 
    public Polygon[] splitAlongGreatMeridian(){
	Polygon[] half_polies;
	if (zero_crossing){
	    half_polies= new Polygon[2];
	    half_polies[0]=(Polygon)leftPoly.clone();
	    half_polies[1]=(Polygon)rightPoly.clone();
	} else {
	    half_polies= new Polygon[1];
	    half_polies[0]=(Polygon)this.clone();
	}
	return half_polies;
    }
    /**
     * Gives the number of vertices of this <tt>Polygon</tt>.
     * @return the number of vertices in this <tt>Polygon</tt>.
     */
    public int vertexCount(){
	return coordinates.length;
    }
    /**
     * Creates an array of lengths of sides of this <tt>Polygon</tt>
     * @return an array of lengths of sides of this <tt>Polygon</tt>.
     */
    public int[] getSides(){
	int[] sides=new int[coordinates.length];
	for (int i=0;i<coordinates.length-1;i++){
	    sides[i]=(int)GeoUtilsBD.distance(coordinates[i],coordinates[i+1]);
	}
	sides[coordinates.length-1]=(int)GeoUtilsBD.distance(coordinates[coordinates.length - 1],coordinates[0]);
	return sides;
    }
    /** Creates an array of locations of vertices of this <tt>Polygon</tt>.
     * @return an array of Point2D.Double objects representing the locations of vertices of this <tt>Polygon</tt>.
     */
    ...
    /**
     * Returns a String representation of this <tt>Polygon</tt>.
     * @return a String representation of this <tt>Polygon</tt> in a form
    ...
    /**
     * Returns a <tt>String</tt> representation of this <tt>Polygon</tt>, but with longitudes translated to 
     ...
     * @return <tt>String</tt> representation of this <tt>Polygon</tt> in a format identical to standard toString() method.
     ...
     * @return <tt>Object</tt> that is an exact copy of this <tt>Polygon</tt>.
     */
    public Object clone(){
	...
	if (coordinates.length>1){
	    ...
	    if (result){
		//set up half-polies
		Vector <Point2D> coordinatesL = new Vector <Point2D> ();
		Vector <Point2D> coordinatesR = new Vector <Point2D> ();
		...
		for (int i=1;i<=coordinates.length;i++){
			...
			double distanceP1GM=GeoUtilsBD.distance(p1,new Point2D.Double(lat1,0));
			double distanceGMP2=GeoUtilsBD.distance(new Point2D.Double(lat2,0),p2);
			double lat=((distanceP1GM/(distanceP1GM+distanceGMP2))*(lat2-lat1))+lat1;
			//System.out.println("Lat :"+lat);
			Point2D.Double cross_point = new Point2D.Double(lat,0);
//			GeoUtils.toWHFormat(cross_point);
		...
		leftPoly = new Polygon(left_poly_array);
		rightPoly= new Polygon(right_poly_array);
	    }
	}
	return result;
    }
...
//	double angle = Polygon.half_diagonal_angle(wid,len);
